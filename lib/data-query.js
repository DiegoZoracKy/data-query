...
var dataQuery = (function() {
    ...
    function checkValue(srcValue, filterValue) {
        ...
        if (srcValue.constructor === Date && filterValue.constructor === Date) {

            return srcValue.getTime() === filterValue.getTime();

        } else if (filterValue.constructor == Function) {
            ...
            return srcValue.every(function(v, i) {
                return (filterValue[i].constructor == RegExp) ? !!v.toString().match(filterValue[i]) : filterValue[i] == v;
            });
